...
const SkillCard = ({ name, icon }: { name: string; icon: string }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-2 bg-white rounded-lg px-4 py-2 shadow-sm hover:shadow-md transition-all duration-300"
    >
...
const SkillSection = ({ title, skills }: { title: string; skills: { name: string; icon: string }[] }) => {
  ...
  return (
    <motion.div
      ...
      animate={inView ? { opacity: 1, y: 0 } : {}}
      transition={{ duration: 0.5 }}
      className="bg-primary-50/50 rounded-xl p-6 backdrop-blur-sm"
    >
      <h3 className="text-2xl font-semibold text-primary-800 mb-6">{title}</h3>
...
const Skills = () => {
  return (
    <section id="skills" className="py-20 bg-gradient-to-b from-white to-primary-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-primary-800">Skills</h2>
